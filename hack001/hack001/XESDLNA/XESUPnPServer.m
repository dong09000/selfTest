...
@interface XESUPnPServer ()<GCDAsyncUdpSocketDelegate>

@property (nonatomic, strong) GCDAsyncUdpSocket *udpSocket;

// key: usn(uuid) string,  value: device
@property (nonatomic, strong) NSMutableDictionary<NSString *, XESUPnPDevice *> *deviceDictionary;
...
- (void)addDevice:(XESUPnPDevice *)device forUSN:(NSString *)usn
{
    if (!device){
        return;
    }
...
- (XESUPnPDevice *)getDeviceWithLocation:(NSString *)location withUSN:(NSString *)usn
{
    dispatch_semaphore_t seamphore = dispatch_semaphore_create(0);
    
    __block XESUPnPDevice *device = nil;
        ...
        if (error) {
            ...
            if (response != nil && data != nil) {
                ...
                if ([httpResponse statusCode] == 200) {
                    device = [[XESUPnPDevice alloc] init];
                    device.location = URL;
                    device.uuid = usn;
