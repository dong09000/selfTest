...
@interface XESDLNA()<XESUPnPServerDelegate, XESUPnPResponseDelegate>

@property(nonatomic,strong) XESUPnPServer *upd;              //MDS服务器
...
@implementation XESDLNA

+ (instancetype)sharedManager{
    static XESDLNA *instance = nil;
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        self.upd = [XESUPnPServer shareServer];
 ...
 */
- (void)startDLNA{
    [self initXESUPnPRendererAndDlnaPlay];
}
/**
 ** DLNA投屏
 ** 【流程: 停止 ->设置代理 ->设置Url -> 播放】
...
- (void)startDLNAAfterStop{
    XESStopAction *action = [[XESStopAction alloc]initWithDevice:self.device Success:^{
        [self initXESUPnPRendererAndDlnaPlay];
...
/**
 初始化XESUPnPRenderer
 */
-(void)initXESUPnPRendererAndDlnaPlay{
    self.render = [[XESUPnPRender alloc] initWithModel:self.device];
...
#pragma mark - XESUPnPResponseDelegate
